...
''' Spatial Detection with YoloV5 example
...
# Get argument first
nnBlobPath = str((Path(__file__).parent / Path('../models/yolov5/bud500Epoch_openvino_2022.1_5shave.blob')).resolve().absolute())
if 1 < len(sys.argv):
    arg = sys.argv[1]
else:
    print("Using YoloV5 model. To use other yolos/mobilenet, use a different script")
...
def run_scissors(distance_mm):
    ...
    if distance_mm <= 400: # distance between camera and scissors
        Arm.Arm_serial_servo_write6(90, 135, 0, 50, 90, 120, 500) # this sets all six servos in trim position
        time.sleep(.5)
        print("open") # lower number will open scissors, higher number will close scissors
        Arm.Arm_serial_servo_write(6, 25, time_1) # 6 is the servo scissors are connected to, it is the claw, 25 is minimum angle for scissors to open properly
        time.sleep(.5)
        print("close") # close scissors
        Arm.Arm_serial_servo_write(6, 120, time_1) # 120 is maximum angle to close scissors properly
        time.sleep(.5)
